...
class PrescriptionController extends Controller{
    ...
    public function create(Request $request){
        ...
        $selected=$_COOKIE['selected_patient'] ?? '';
        
        setcookie('selected_patient','');
        if(!$selected)
        {
    	    return view('prescription.create',['drugs' => $drugs, 'patients' => $patients, 'tests' => $tests]);
        }
    	
        return view('prescription.create',['selected'=>$selected,'drugs' => $drugs, 'patients' => $patients, 'tests' => $tests]);
     
    }
    public function redirect(Request $request,$res){
       $selected= setcookie("selected_patient",$res);
       return redirect()->route('prescription.create');
    }
